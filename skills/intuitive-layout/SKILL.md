...
- Preserve behavior, not old paths. Migrate runnable entrypoints, import paths,
  public command paths, and documented examples to the new intuitive layout.
- Remove stale wrappers, aliases, old script locations, and compatibility
  entrypoints by default after known consumers are updated.
...
For an approved or precise layout slice, default to aggressive cleanup:

- choose the clearer new folder/API shape over compatibility with old paths
- update in-repo consumers, docs, tests, recipes, examples, and CI to the new
  shape
- delete old wrappers and entrypoints unless the user explicitly protects them
  or a real external contract must remain live
- document migration notes when old commands or imports intentionally break
